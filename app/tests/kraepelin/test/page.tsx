...
//     if (direction === "next") {
//       // targetCol selalu activeCol + 1, tanpa batasan systemActiveCol
//       const targetCol = activeCol + 1;
//       if (targetCol >= COLS) return;

//       // Jika maju melampaui systemActiveCol, advance systemActiveCol pula
//       if (targetCol > systemActiveCol) {
//         if (isTimedOut) {
//           // Pindah sah: waktu habis, advance systemActiveCol
//           setActiveCol(targetCol);
//           setSystemActiveCol(targetCol);
...
//         }
//       } else {
//         // Masih dalam jangkauan lajur yang sudah dikunjungi sebelumnya (activeCol < systemActiveCol)
//         // Tetap catat sebagai pelanggaran karena bukan lajur legitimate
//         setActiveCol(targetCol);
//         const firstEmpty = answers[targetCol].findIndex(a => a === null);
//         const targetPair = firstEmpty !== -1 ? firstEmpty : 0;
//         setFocusedInput({ col: targetCol, pair: targetPair });
...
//                 ? "🚨 Terlambat pindah lajur! Pindah lajur sekarang!"
//                 : colStates[systemActiveCol].timedOut && graceTimeLeftMs !== null
//                 ? `⏰ Silahkan pindah lajur! ${Math.ceil(graceTimeLeftMs / 1000)}d tersisa`
//                 : "⚠ Waktu hampir selesai, Bersiap pindah ke lajur berikutnya!"}
//             </div>
//           )}
              ...
              <div className="flex gap-2 w-full">
                ...
                {activeCol >= COLS - 1 ? (
                  <button
                    ...
                    className="flex-1 bg-green-600 hover:bg-green-700 active:bg-green-800 text-white font-bold text-sm py-2 rounded-lg shadow transition-all"
                  >
                    Submit ✓
                  </button>
                ) : (
                  <button
                    onClick={() => handleMoveColumn("next")}
                    className="flex-1 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-bold text-sm py-2 rounded-lg shadow transition-all"
                  >
                    Next →
                  </button>
                )}
